function handSubmit(event) {
    ...
    if(Client.validURL(JSON.parse(JSON.stringify(input_url[0].value))))
    {
        ...
        .then(function(res) {
            ...
            document.querySelector('section.url-results #pol').innerHTML = res.polarity
            document.querySelector('section.url-results #sub').innerHTML = res.subjectivity
            document.querySelector('section.url-results #pol_con').innerHTML = res.polarity_confidence
            document.querySelector('section.url-results #sub_con').innerHTML = res.subjectivity_confidence
            document.querySelector('section.url-results #ex').innerHTML = res.text
        })
